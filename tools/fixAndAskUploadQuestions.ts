...
// --- HELPERS ---
function inferCategory(filePath: string, topic: string): string {
  // Try to infer from folder name (e.g., .../questions/anatomy/...)
  const match = filePath.match(/questions\/(\w+)/i);
  if (match && match[1]) {
    return match[1].replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase());
  }
  // Fallback to topic if available
  if (topic && typeof topic === "string") {
    return topic.replace(/_/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase());
  }
  return "Uncategorized";
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function fixQuestion(q: any, topicFromFolder: string, filePath: string): any {
  const now = new Date().toISOString();
  // --- Ensure all required analytics/metadata fields ---
  if (!q.id || typeof q.id !== "string") { q.id = (q.question || "").slice(0, 32) + Math.random().toString(36).slice(2, 8); }
  if (!q.question) { q.question = ""; }
  if (!Array.isArray(q.answers)) { q.answers = []; }
  if (typeof q.correct !== "number") { q.correct = 0; }
  if (!Array.isArray(q.incorrect)) { q.incorrect = []; }
  if (!Array.isArray(q.keywords)) { q.keywords = []; }
  if (!Array.isArray(q.tags)) { q.tags = []; }
  if (!q.topic) { q.topic = topicFromFolder; }
  // --- Auto-fix category ---
  ...
  if (!("last_answered_at" in q)) { q.last_answered_at = null; }
  if (!("difficulty_adjustment" in q)) { q.difficulty_adjustment = 0; }
  if (!("created_at" in q)) { q.created_at = now; }
  q.updated_at = now;
