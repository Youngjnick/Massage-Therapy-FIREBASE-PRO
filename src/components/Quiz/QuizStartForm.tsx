...
const QuizStartForm: React.FC<QuizStartFormProps> = (props) => {
  // Prefer new signature if present, else fallback to legacy
  const toggleState = props.toggleState ?? {
    showExplanations: props.showExplanations ?? true,
    instantFeedback: props.showInstantFeedback ?? true,
    randomizeQuestions: props.randomizeQuestions ?? true,
    randomizeOptions: props.randomizeOptions ?? false,
  };
  const setToggleState = props.setToggleState ?? ((next: any) => {
    props.setShowExplanations?.(next.showExplanations);
    props.setShowInstantFeedback?.(next.instantFeedback);
    props.setRandomizeQuestions?.(next.randomizeQuestions);
    props.setRandomizeOptions?.(next.randomizeOptions);
  });
  ...
  return (
    <form data-testid="quiz-start-form" style={{ marginBottom: '1rem' }} onSubmit={e => { e.preventDefault(); props.onStart({ selectedTopic: props.selectedTopic, quizLength: props.quizLength, sort: props.sort, ...toggleState }); }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', margin: '1rem 0' }}>
        <div>
          ...
          <input
            ...
            onChange={e => {
              setToggleState({ ...toggleState, showExplanations: e.target.checked });
              if (props.setShowExplanations) props.setShowExplanations(e.target.checked);
            }}
            onKeyDown={e => {
              if (e.key === ' ' || e.key === 'Enter') {
                e.preventDefault();
                const newValue = !toggleState.showExplanations;
                setToggleState({ ...toggleState, showExplanations: newValue });
                if (props.setShowExplanations) props.setShowExplanations(newValue);
              }
            }}
            style={{ marginLeft: 6 }}
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={e => {
              setToggleState({ ...toggleState, instantFeedback: e.target.checked });
              if (props.setShowInstantFeedback) props.setShowInstantFeedback(e.target.checked);
            }}
            style={{ marginLeft: 6 }}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={e => {
              setToggleState({ ...toggleState, randomizeQuestions: e.target.checked });
              if (props.setRandomizeQuestions) props.setRandomizeQuestions(e.target.checked);
            }}
            style={{ marginLeft: 6 }}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={e => {
              setToggleState({ ...toggleState, randomizeOptions: e.target.checked });
              if (props.setRandomizeOptions) props.setRandomizeOptions(e.target.checked);
            }}
            style={{ marginLeft: 6 }}
          />
        </div>
      ...
      <button style={{ marginLeft: '1rem' }} type="submit" disabled={props.quizLength <= 0}>Start Quiz</button>
    </form>
  );
};
