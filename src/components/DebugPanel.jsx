...
export default function DebugPanel({ appState, visible }) {
  ...
  const user = appState.user || (window && window.appState && window.appState.user);
  const firestoreStatus = (analytics && user && user.uid) ? 'Synced' : 'Not Synced';
  ...
  if (analytics && firestoreAnalytics) {
    try {
      analyticsMatch = JSON.stringify(analytics) === JSON.stringify(firestoreAnalytics);
    } catch {}
  }

  // Always visible in E2E/dev unless explicitly hidden
  const isE2E = typeof window !== 'undefined' && (window.__E2E_TEST__ || import.meta?.env?.MODE === 'development' || import.meta?.env?.VITE_E2E);
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 13, marginBottom: 8 }}>
        <b>Analytics Firestore Sync:</b> <span style={{ color: firestoreStatus === 'Synced' ? '#090' : '#c00' }}>{firestoreStatus}</span>
        {lastSync && (
          <span style={{ marginLeft: 10, color: '#888' }}>(Last Sync: {new Date(lastSync).toLocaleString()})</span>
        )}
        {firestoreAnalytics && (
          <span style={{ marginLeft: 10, color: analyticsMatch ? '#090' : '#c00' }}>
            {analyticsMatch ? 'Analytics match Firestore' : 'Analytics differ from Firestore'}
          </span>
        )}
      </div>
      <pre
