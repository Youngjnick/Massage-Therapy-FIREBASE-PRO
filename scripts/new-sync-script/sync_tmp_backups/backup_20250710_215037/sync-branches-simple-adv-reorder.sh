...
select_branches() {
  ...
  if [[ ${#selected_branches[@]} -eq 0 ]]; then
    # No branches selected, use current branch as default
    local current_branch
    current_branch=$(git rev-parse --abbrev-ref HEAD)
    info "No branches selected. Using current branch: $current_branch"
    selected_branches=("$current_branch")
  fi
    ...
    if (( $# == 0 )); then
      ...
      for num in $flag_nums; do
        case $num in
          1) export NO_TESTS=1 ;;
          2) export NO_LINT=1 ;;
          3) export NO_VERIFY=1 ;;
          4) export DRY_RUN=1 ;;
          5) export AUTO_COMMIT=1 ;;
          6) export DEBUG=1 ;;
          7) export REBASE=1 ;;
          8) export NO_PLAYWRIGHT=1 ;;
          9) print_help; exit 0 ;;
        esac
      done
    fi
