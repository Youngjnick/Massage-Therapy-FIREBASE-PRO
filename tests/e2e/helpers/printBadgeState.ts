...
export async function printBadgeState(page: Page, label = '') {
  await page.evaluate((label) => {
    ...
    console.log(`%c[E2E][BADGE STATE] ${label}\nwindow.appState.badges:`, 'color: #0074D9', JSON.stringify(window.appState && window.appState.badges));
    // eslint-disable-next-line no-console
    console.log(`%c[E2E][BADGE STATE] ${label}\nwindow.badges:`, 'color: #2ECC40', JSON.stringify(window.badges));
    // eslint-disable-next-line no-console
    console.log(`%c[E2E][BADGE STATE] ${label}\nwindow.earnedBadges:`, 'color: #FF4136', JSON.stringify(window.earnedBadges));
  }, label);
}
